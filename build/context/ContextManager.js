...
export class ContextManager {
    ...
    async createContext(code, language, framework, dependencies) {
    ...
    async updateContext(contextId, newContext) {
    ...
    async pruneContext(contextId) {
        this.activeContexts.delete(contextId);
        this.contextCache.del(contextId);
    }
    async calculateRelevance(oldContext, newContext) {
        ...
        if (newContext.dependencies && oldContext.dependencies) {
            const depsMatch = Object.entries(newContext.dependencies).every(([key, value]) => oldContext.dependencies?.[key] === value);
        ...
        if (newContext.fileContext && oldContext.fileContext) {
            ...
            const similarity = this.calculateStringSimilarity(newContext.fileContext, oldContext.fileContext);
    ...
    async shareContext(contextId, targetExperts) {
