...
class CodeReviewMCP {
    ...
    setupRequestHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            const { code, language, framework } = request.params.arguments;
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        });
        // Error handling
    ...
    async analyzeCode(code, language, framework) {
        // Create context for this analysis
        const contextId = await this.contextManager.createContext(code, language, framework);
        // Get context
        ...
        const selectedExperts = await this.gatingNetwork.selectExperts(language, characteristics);
        // Perform analysis with selected experts
        const analysisPromises = selectedExperts.map((expert) => expert.analyze(code, context));
